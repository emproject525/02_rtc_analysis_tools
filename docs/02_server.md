...
- **연결 직후 초기 스냅샷 1회**: 현재 hub의 모든 peer `latest`를 `snapshot` 이벤트로
  먼저 보낸다. (없으면 대시보드를 새로 열 때 다음 Report가 올 때까지 빈 화면)
- 이후 델타를 스트리밍:
  - `report` 이벤트 — 새 Report 1건 (peer 갱신/신규)
  - `gone` 이벤트 — `{ peerId }` evict 통지
- 이벤트 형식: `event: <kind>\n` + `data: <json>\n\n`.
...
Map<peerId, {
  ...
  lastSeenAt: number; // 마지막 수신 시각 (Date.now) — stale evict 기준
}>
```
...
peer가 그냥 끊기면(탭 닫힘 등) 마지막 신호가 안 올 수 있다. 주기 sweep(예: 10초)으로
`now - lastSeenAt > 30초`인 peer를 hub에서 제거하고 `gone` 이벤트를 푸시해 대시보드의
"유령 연결"을 막는다.
...
```ts
hub.push(report: Report): void           // 수집 → latest 교체 + series ring + 구독자 통지
hub.snapshot(): Report[]                  // 현재 모든 peer latest (SSE 초기 스냅샷)
...
- `/` 페이지에서 `EventSource('/api/stream')` 구독 → 초기 `snapshot`으로 현재 연결을
  그리고, `report`/`gone` 델타로 라이브 갱신.
- **MVP는 표 기반 라이브 갱신**(런타임 의존성 0): peer 목록 + 선택 peer 상세(connection
  상태, transport 경로, recv/send 트랙 지표). `ended` 트랙은 흐름값이 비어 있으므로
  구분 표시(예: 흐리게).
- 그래프(시계열 차트)는 `series`를 활용하는 **스트레치** — 차트 라이브러리 추가 필요.
...
| 인증 | 검증 안 함 (non-goal) |
| hub 모델 | `Map<peerId, { latest, series, lastSeenAt }>`, `globalThis` 싱글톤 |
| 버퍼 N | peer당 300 ring buffer (shift+push) |
| stale | 30초 무수신 evict + `gone` 통지 |
| SSE | 연결 시 `snapshot` 1회 → `report`/`gone` 델타 |
| CORS | `OPTIONS` 처리 + `*`(env 화이트리스트) + `authorization` 허용 |
| UI | 표 기반 라이브(의존성 0), 그래프는 스트레치 |
