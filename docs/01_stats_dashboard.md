...
- **track 정보**
  ...
  - mid
  - 코덱 (mimeType) _(clockRate 등은 raw codec stats엔 있으나 Report엔 미노출)_
- **receiver 설정**
...
| FPS (없을 때)     | _(미구현)_ 현재는 `framesPerSecond` 패스스루, Δframes/Δt 폴백 없음 |
...
- `ended`로 바뀌면 "현재 흐름" 파생값(`bitrate`·`packetLossRate`·`retransmissionRate`·`fractionLost`·`framesPerSecond`·`jitterBufferDelay`)은 비운다(`undefined`) — 더 이상 흐르지 않으므로. 누적/정적값(ssrc·codec·해상도·누적 카운터)은 마지막 관측치로 남긴다.
